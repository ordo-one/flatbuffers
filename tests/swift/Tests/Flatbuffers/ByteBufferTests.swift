...
final class ByteBufferTests: XCTestCase {
  ...
  func testRebindOfUniquelyReferencedBufferReusesStorage() {
  ...
  func testRebindOfSharedBufferLeavesTheOtherCopyIntact() {
    let first = UnsafeMutableRawPointer.allocate(byteCount: 16, alignment: 8)
    ...
    XCTAssertFalse(byteBuffer.rebind(assumingMemoryBound: second, capacity: 8))
    XCTAssertEqual(byteBuffer.read(def: UInt32.self, position: 4), 0xBBBB_0002)
